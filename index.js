...
// using path module to locate files with a json extension name
...
async function main() {
  ...
  // create the salesTotal directory if it doesn't exist(the 'try' logs the error message to the console)
  try {
  ...
  // find paths to all the sales files (using the findSalesFiles function created earlier)
  const salesFiles = await findSalesFiles(salesDir);
  ...
  await fs.writeFile(
    ...
    "we just created a file and wrote words on it without actually creating a file...lol"
  );
  await fs.writeFile(
    path.join(salesTotalsDir, "totals.txt"),
    `${salesTotal}\r\n`,
